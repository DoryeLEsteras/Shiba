...
from Shiba.h_reader import WannierHamiltonian
...
if __name__ == '__main__':
   ...
   print(WannierParameters.read_wannier_info(Parameters.hfile,Parameters.nc))
...
def mode_detector(h_list:List[str])
    dim = len(h_list)
    if dim == 1:
       calculation_mode = 1
    elif dim == 2:
       ...
       logger('\nIncompatible number of input files, exiting.' + '\n')
    parameters.calculation_mode = calculation_mode
    degs = []
...
dbl=8
cdbl=16
Mmels=norb*norb*nws*dbl
Mh=(blocks*4*(norb//2))*(blocks*4*(norb//2))*cdbl
MId=blocks*4*(norb//2)*cdbl
MIdnorb2=(norb//2)*cdbl
Mhuu=(norb//2)*(norb//2)*cdbl
Mgam1=(4*(norb//2))*(4*(norb//2))*cdbl
Mspec=vpts*dbl*3+Mh*3
Mcond=vpts*dbl*4+Mh*5
Mtot=Mmels+Mh*24+MId*2+MIdnorb2+Mhuu*4+Mgam1*2+(Mspec*(spec==1))+Mcond
